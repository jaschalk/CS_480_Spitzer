...
def graph_results():
    ...
    custom_moving_average_trick_points_graph_data = [[],[]]
    learning_moving_average_list = []
    learning_moving_average_trick_points_graph_data = [[],[]]
...
#            print(f"{agent_win_counts[0]/game_count}  {agent_win_counts[1]/game_count}  {agent_win_counts[2]/game_count}")
            ...
            random_agent_winrate_graph_data[0].append(game_count)
            random_agent_winrate_graph_data[1].append(random_agent_win_ratio)
            custom_agent_win_ratio = agent_win_counts[1]/game_count
            custom_agent_winrate_graph_data[0].append(game_count)
            custom_agent_winrate_graph_data[1].append(custom_agent_win_ratio)
            ...
            agent_average_trick_points_graph_data = [learning_average_trick_points_graph_data, 
                                                    custom_average_trick_points_graph_data, 
                                                    random_average_trick_points_graph_data]
            agent_moving_average_trick_points_lists = [learning_moving_average_list,
                                                        custom_moving_average_list,
                                                        random_moving_average_list]
            agent_moving_average_trick_points_graph_data = [learning_moving_average_trick_points_graph_data,
                                                             custom_moving_average_trick_points_graph_data, 
                                                             random_moving_average_trick_points_graph_data]
                    ...
                    for player_id in range(4):
                        for agent_type_index in range(3):
                            if player_id in agent_numbers_lists[agent_type_index]: # this feels like it's wrong right now, not sure why
                                ...
                                agent_moving_average_trick_points_lists[agent_type_index].append(points_taken)
                                if len(agent_moving_average_trick_points_lists[agent_type_index]) >= moving_average_size:
                                    agent_moving_average_trick_points_graph_data[agent_type_index][0].append(total_tricks_played)
                                    agent_moving_average_trick_points_graph_data[agent_type_index][1].append(sum(agent_moving_average_trick_points_lists[agent_type_index])/moving_average_size)
                                    agent_moving_average_trick_points_lists[agent_type_index].pop(0)

    fig = go.Figure()
    fig.update_layout(title="Average Winrate")
    fig.add_trace(go.Scatter(x=learning_agent_winrate_graph_data[0], y=learning_agent_winrate_graph_data[1], name='Learning Agent'))
    fig.add_trace(go.Scatter(x=custom_agent_winrate_graph_data[0], y=custom_agent_winrate_graph_data[1], name='Custom Agent'))
    fig.add_trace(go.Scatter(x=random_agent_winrate_graph_data[0], y=random_agent_winrate_graph_data[1], name='Random Agent'))
    fig.show()

    trick_average_graph = go.Figure()
    trick_average_graph.update_layout(title="Average Points per Trick")
    ...
    moving_average_graph.update_layout(title="Moving Average Points per Trick")
    for i in range(3):
        moving_average_graph.add_trace(go.Scatter(x=agent_moving_average_trick_points_graph_data[i][0], y=agent_moving_average_trick_points_graph_data[i][1], name=graph_labels[i]))
    moving_average_graph.show()
